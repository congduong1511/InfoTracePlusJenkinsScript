...
pipeline {
	...
	stages{
		...
		stage('ReleaseStaticECSSSShared') { 
			environment{
		...
		stage('ReleaseSPSSSSShared') { 
			...
			when{expression {return devenv['SSSShared_ReleaseSPS'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('ReleaseSPSBuild') { 
			...
			when{expression {return devenv['SPS_Release'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('MAILSEND'){
			environment{
				...
				MAILTO="${devenv['BUILDERRMAILTOSPS']}"
			}
