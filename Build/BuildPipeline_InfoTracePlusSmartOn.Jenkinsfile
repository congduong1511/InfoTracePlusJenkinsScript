...
pipeline {
	...
	stages{
		...
		stage('ReleaseSmaoSSSShared') { 
			...
			when{expression {return devenv['SSSShared_ReleaseSmao'] == 'ON'}}
			steps{
				script{
		...
		stage('ReleaseSmartOnBuild') { 
			...
			when{expression {return devenv['SmartOn_Release'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('MAILSEND'){
			environment{
				...
				MAILTO="${devenv['BUILDERRMAILTOSO']}"
			}
