...
pipeline {
	...
	stages{
		...
		stage('ReleaseSRMSSSShared') { 
			...
			when{expression {return devenv['SSSShared_ReleaseSRM'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('ReleaseSRMBuild') { 
			...
			when{expression {return devenv['SRM_Release'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
					...
					def buildreturn = 0
					// ビルドを実行する。PLATFORMwin32 or x64 でWin32とx64でのビルドを行う
					buildreturn += msbuildbat.MsBuild(MSBUILD, SOLUTION, SOLUTIONFILE, CONFIG, "Mixed Platforms", BUILDLOG, ISREBUILD.toBoolean())
					if(buildreturn < 0){
						BUILDERROR = 'ON'
						currentBuild.result = 'FAILURE'
					}
		...
		stage('MAILSEND'){
			environment{
				...
				MAILTO="${devenv['BUILDERRMAILTOSRM']}"
			}
