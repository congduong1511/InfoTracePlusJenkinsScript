...
pipeline {
	...
	stages{
		...
		stage('ReleaseSSSAgtSSSShared') { 
			...
			when{expression {return devenv['SSSShared_ReleaseSSSAgt'] == 'ON'}}
			steps{
				script{
		...
		stage('UnicodestaticReleaseSSSAgtBuild') { 
			environment{
				// config設定 例: "Release Static EC"
			...
			when{expression {return devenv['SSSAgt_Release'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('ECAgtReleaseBuild') {
			environment{
				CONFIG = "Release"
				SOLUTIONFILE = 'ECAgt'
				SOLUTION = "${BUILDPATH}\\${SOLUTIONFILE}.sln"
			}
			when{expression {return devenv['ECAgt_Release'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('MAILSEND'){
			environment{
				...
				MAILTO="${devenv['BUILDERRMAILTOSSSAGT']}"
			}
