...
pipeline {
	...
	stages{
		...
		stage('ReleaseSSSToolsBuild') { 
			environment{
				// config設定 例: "Release Static EC"
				CONFIG = 'Release' 
				// ソリューション設定(.slnは不要) 例: 'SSSShared'
				SOLUTIONFILE = 'SSSTools' 
				SOLUTION = "${BUILDPATH}\\${SOLUTIONFILE}.sln"
			}
			when{expression {return devenv['SSSTools_Release'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('ReleaseSSSDiagBuild') { 
			environment{
				// config設定 例: "Release Static EC"
				CONFIG = 'Release' 
				// ソリューション設定(.slnは不要) 例: 'SSSShared'
				SOLUTIONFILE = 'SSSDiag' 
				SOLUTION = "${BUILDPATH}\\${SOLUTIONFILE}.sln"
			}
			when{expression {return devenv['SSSDiag_Release'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('MAILSEND'){
			environment{
				...
				MAILTO="${devenv['BUILDERRMAILTOTOOLS']}"
			}
