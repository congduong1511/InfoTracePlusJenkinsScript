...
pipeline {
	...
	stages{
		...
		stage('ReleaseIntegrateAgtBuild') { 
			environment{
				// config設定 例: "Release Static EC"
				CONFIG = 'Release' 
				// ソリューション設定(.slnは不要) 例: 'SSSShared'
				SOLUTIONFILE = 'IntegrateAgt' 
				SOLUTION = "${BUILDPATH}\\${SOLUTIONFILE}.sln"
			}
			when{expression {return devenv['IntegrateAgt_Release'] == 'ON'}}
			steps{
				script{
		...
		stage('ReleaseIntegrateMgrBuild') { 
			environment{
				// config設定 例: "Release Static EC"
				CONFIG = 'Release' 
				// ソリューション設定(.slnは不要) 例: 'SSSShared'
				SOLUTIONFILE = 'IntegrateMgr' 
				SOLUTION = "${BUILDPATH}\\${SOLUTIONFILE}.sln"
			}
			when{expression {return devenv['IntegrateMgr_Release'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('MAILSEND'){
			environment{
				...
				MAILTO="${devenv['BUILDERRMAILTOINST']}"
			}
