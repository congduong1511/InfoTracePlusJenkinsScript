...
pipeline {
	...
	stages{
		...
		stage('ReleaseSDMCSSSShared') { 
			...
			when{expression {return devenv['SSSShared_ReleaseSDMC'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('ReleaseSDMCECShared') { 
			...
			when{expression {return devenv['ECShared_ReleaseSDMC'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('ReleaseSDMCITShared') { 
			environment{
				// config設定 例: "Release Static EC"
				CONFIG = 'Release SDMC' 
				// ソリューション設定(.slnは不要) 例: 'SSSShared'
				SOLUTIONFILE = 'ITShared' 
				SOLUTION = "${BUILDPATH}\\${SOLUTIONFILE}.sln"
			}
			when{expression {return devenv['ITShared_ReleaseSDMC'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('ReleaseSDMCBuild') { 
			...
			when{expression {return devenv['SDMC_Release'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('ReleaseSDMCSmartOn') { 
			...
			when{expression {return devenv['SmartOn_ReleaseSDMC'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('MAILSEND'){
			environment{
				...
				MAILTO="${devenv['BUILDERRMAILTOSDMC']}"
			}
