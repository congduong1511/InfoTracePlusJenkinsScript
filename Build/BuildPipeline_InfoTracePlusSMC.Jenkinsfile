...
pipeline {
	...
	stages{
		...
		stage('ReleaseSMCSSSShared') { 
			...
			when{expression {return devenv['SSSShared_ReleaseSMC'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('ReleaseSMCECShared') { 
			...
			when{expression {return devenv['ECShared_ReleaseSMC'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('ReleaseSMCBuild') { 
			...
			when{expression {return devenv['SMC_Release'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('ReleaseSMCSmartOn') { 
			...
			when{expression {return devenv['SmartOn_ReleaseSMC'] == 'ON'}}
			steps{
				script{
					if("${ITPVERSION}" != '320' && "${BETA}" != "Beta0"){
						CONFIG += " ${BETA}"
		...
		stage('MAILSEND'){
			environment{
				...
				MAILTO="${devenv['BUILDERRMAILTOSMC']}"
			}
